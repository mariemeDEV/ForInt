...
function generateProjectId($length):string{
    $characters       = '0123456789abcdefABCDEFGHIJKLMNOPQRSTWXYZ@';
    $charactersLength = strlen($characters);
    $idproject         = '';
    $password         = '';
    for ($i = 0; $i < $length; $i++) {
        $idproject  .= $characters[rand(0, $charactersLength - 1)];
    }
    return  $idproject ;
}

$valueNumP = $_SESSION['matricule'].'/'.generateProjectId(3);

$datePolice = date_create()->format('Y-m-d H:i:s');
if($c1==1 && $c2==1 && $c3==1 && $c4==1 && $c5==1 && $c6==1 )
{
    $usdao             = new PoliceDao();
    $us                = new Police($unikId,$valueNumP,$datePolice,'','NULL',0,'En cours',$_SESSION['matricule'],$unikId,$unikId,$unikId,$unikId,$unikId,$unikId,'NULL');
